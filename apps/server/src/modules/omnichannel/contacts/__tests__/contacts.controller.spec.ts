...
describe('ContactsController (Presentation Layer Endpoints)', () => {
  ...
  beforeEach(() => {
    mockContactsService = {
      ...
      findIdentitiesByContactId: async (_workspaceId: string, _contactId: string) => [
        sampleIdentity,
      ],
      linkIdentity: async (_workspaceId: string, contactId: string, dto: any) => ({
        ...sampleIdentity,
        contactId,
        channelId: dto.channelId,
        externalContactId: dto.externalContactId,
      }),
      unlinkIdentity: async (_workspaceId: string, _contactId: string, _identityId: string) => ({
        success: true,
      }),
    };
  ...
  describe('Contact Merge Endpoint (Feature F-1.2.4)', () => {
    it('should delegate merge action to ContactsService and pass performedByUserId', async () => {
