...
export class OrderExtractorService {
  ...
  async extractOrderFromMessage(
    ...
    // If confidence is >= 80%, emit real-time event for 1-click Commerce suggestion
    if (confidenceScore >= 80) {
      ...
      this.logger.log(
        `High confidence (${confidenceScore}%) Commerce draft suggested for conv ${conversationId}`,
      );
    }
