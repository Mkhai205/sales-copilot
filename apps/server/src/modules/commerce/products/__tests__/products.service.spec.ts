...
describe('ProductsService (Catalog & Inventory Management)', () => {
  ...
  describe('updateProduct', () => {
    it('should update product attributes and existing variant', async () => {
      const prod = await service.createProduct(ws1, {
        name: 'Original Name',
        sku: 'ORIG-SKU',
        basePrice: 100000,
        variants: [{ name: 'V1', sku: 'ORIG-V1', price: 100000, stockQuantity: 5 }],
      });

      const updated = await service.updateProduct(ws1, prod.id, {
        name: 'Updated Name',
        basePrice: 120000,
    ...
    it('should emit INVENTORY_UPDATED when adding a new variant with initial stock in updateProduct', async () => {
      const prod = await service.createProduct(ws1, {
        name: 'Product For Variant Expansion',
        sku: 'EXPAND-SKU',
        basePrice: 200000,
        variants: [{ name: 'V1', sku: 'EXPAND-V1', price: 200000, stockQuantity: 5 }],
      });
