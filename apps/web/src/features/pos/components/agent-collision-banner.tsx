...
export function AgentCollisionBanner({
  ...
  const agentDisplay = lockedBy.userName || lockedBy.userEmail || t('pos.collision.otherAgent');
      ...
      <div className="flex items-start gap-2.5 min-w-0">
        ...
        <div className="flex flex-col min-w-0">
          <span className="font-semibold text-xs leading-tight">
            {t('pos.collision.orderLockedBy', { agent: agentDisplay })}
          </span>
          <span className="text-[11px] text-amber-700/80 dark:text-amber-400/80 mt-0.5 truncate">
            {t('pos.collision.autoReleaseNotice', { seconds: remainingTtlSeconds })}
          </span>
        </div>
      </div>
